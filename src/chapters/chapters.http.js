...
const register = (req, res) => {
    ...
    if (!Object.keys(data)) {
        ...
        return res.status(400).json({
            message: 'You are missing fields of info', fields: {
                chapter_num: 'number'
            }
        })
    } else {
        const body = chaptersControllers.createChapter(data, programID)
        return res.status(201).json({ message: `Chapter was created succesfully with id: ${body.id}`, chapter: body })
    }
}
...
const edit = (req, res) => {
    ...
    if (typeof(data.chapter_num) !== 'number') {
        return res.status(400).json({
            message: 'You need to complete all fields', fields: {
                chapter_num: 'number'
            }
        })
    } else {
